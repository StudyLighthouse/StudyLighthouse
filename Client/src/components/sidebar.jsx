  ...
  const handleNewChat = () => {
    if (sessionStorage.length > 1) {
      const confirmNewChat = window.confirm(
        "Do you want to start a new chat without saving the current one?"
      );
      if (confirmNewChat) {
        const keysToRemove = [];
        for (let i = 0; i < sessionStorage.length; i++) {
    ...
    <div className="chatComponent bg-zinc-800 w-full h-full">
      <div className="bg-zinc-800 p-2 text-white flex flex-shrink-0 flex-col justify-center items-center md:w-full">
        ...
        <div className="flex flex-col mb-4 pt-7">
          <label className="block text-white mb-2">Save chat as:</label>
          <input
            type="text"
            ...
            {savedChats.map((c, index) => (
              <div key={index} className="w-full border-white mb-1">
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="h-10 text-white w-[100%]"
        ...
        <div className="flex flex-col justify-center items-center p-2 w-full">
          ...
          <div className="messageBox w-full bg-black">
            <div className="fileUploadWrapper">
              ...
              />
              {/* {imageUploaded && toast.info("Image uploaded successfully!")} */}
            </div>
            <input
              required
