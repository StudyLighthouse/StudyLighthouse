    ...
    <div className="max-h-screen w-screen m-0 flex flex-col">
      ...
      <div className="h-1/2 w-full ">
        ...
        <div className="h-full w-full flex flex-col items-center p-4">
          {questionDetails.file_url &&
            questionDetails.question && ( // Check if question has both text and file URL (image)
              <div className="h-full w-full flex flex-col items-center">
                <img
                  src={questionDetails.file_url}
                  alt="Question"
          ...
          {questionDetails.file_url &&
            !questionDetails.question && ( // Check if question has only file URL (image)
              <img
                src={questionDetails.file_url}
                alt="Question"
          ...
          {!questionDetails.file_url &&
            questionDetails.question && ( // Check if question has only text
              <h2 className="text-white">{questionDetails.question}</h2>
            )}
        </div>
      </div>
      ...
      <div className="solution_posting flex-grow flex items-end justify-center w-full lg:flex-row md:flex-col sm:flex-col p-4">
        ...
        <div className="fileUpload  justify-center flex">
          <div className="file-upload-form sm:w-full flex flex-col items-center justify-center rounded-lg">
            <label
              htmlFor="file"
              className="file-upload-label cursor-pointer sm:h-48 sm:w-48 flex flex-col items-center justify-center"
            >
              <div className="file-upload-design sm:w-full flex flex-col items-center">
                ...
                <p className="text-sm sm:w-full md:text-base lg:text-lg sm:text-sm  flex items-center justify-center">
                  Drag and Drop
                </p>
                <p className="sm:text-sm md:text-base lg:text-lg">or</p>
                <span className="browse-button sm:w-4/5 text-blue-500 hover:text-blue-700">
                  Browse file
                </span>
              </div>
              <input
        ...
        {/* </form> */}
        <button
          className="submit-button lg:ml-3 md:mt-3 sm:mt-3"
          type="submit"
